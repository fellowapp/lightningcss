...
pub struct Config<'i> {
  ...
  pub hash_prefix: Option<Cow<'i, str>>,
...
impl<'i> Pattern<'i> {
  ...
  fn parse_hash_segment(raw: &str, start_idx: usize) -> Result<Segment<'i>, PatternParseError> {
...
mod tests {
  ...
  fn first_hash<'a, 'i>(p: &'a Pattern<'i>) -> &'a Segment<'i> {
    p.segments
      .iter()
